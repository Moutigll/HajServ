	...
	while (std::getline(file, line))
	{
		...
		if (line.rfind("allowed_methods", 0) == 0 || line.rfind("server_name", 0) == 0 || line.rfind("index", 0) == 0 || line.rfind("listen", 0) == 0)
		{
			...
			if (!parseVectors(line, server, nullloc, false))
				return false;
			if (hasDuplicates(server._methods) || 
				hasDuplicates(server._hosts) ||
				hasDuplicates(server._indexes) ||
				hasDuplicates(server._ports))
			{
				std::cerr << RED << "Error: Duplicate entries found in server parsing at line " << line_number << "." << RESET << std::endl;
				return false;
			}
			continue;
		}
