...
/*   Updated: 2025/06/28 08:29:10 by etaquet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (std::getline(file, line))
	{
		...
		if (line.rfind("allowed_methods", 0) == 0 || line.rfind("server_name", 0) == 0 || line.rfind("index", 0) == 0 || line.rfind("listen", 0) == 0)
		{
			...
			if (!parseVectors(line, server, nullloc, false))
				return false;
			if (hasDuplicates(server._ports) || hasDuplicates(server._methods) || 
				hasDuplicates(server._hosts) || hasDuplicates(server._indexes))
			{
				std::cerr << RED << "Error: Duplicate found in server parsing at line " << line_number << "." << RESET << std::endl;
				return false;
			}
			continue;
		}
		result = goToOtherParse(file, line_number, line, server);
		if (result == -1) continue;
		if (!result) return false;
		std::istringstream iss(line);
		result = parseServerKeys(iss, line_number, line, server);
		if (result == -1) continue;
		if (!result) return false;
	}
...
bool Config::parseLocation(std::ifstream &file, int &line_number, t_location &loc)
{
	...
	while (std::getline(file, line))
	{
		...
		int result = parseLocationKeys(iss, line_number, line, loc);
		if (result == -1) continue;
...
bool Config::parseErrors(std::ifstream &file, int &line_number, t_server &server)
{
	...
	while (std::getline(file, line))
	{
		...
		if (!(iss >> key >> value) || (iss >> std::ws && !iss.eof()) || atoi(key.c_str()) <= 0)
		{
			std::cerr << RED << "Syntax error at line " << line_number << ": '" << line << "'. No key or value was found." << RESET << std::endl;
			return false;
		}
		if (value[value.length() - 1] == ';')
...
void Config::logConfig() const
{
	...
	for (size_t i = 0; i < _servers.size(); i++)
	{
		...
		out += BRIGHT_YELLOW + std::string("\tClient max body size: ") + BRIGHT_CYAN + to_string(srv._clientMaxBodySize) + " bytes" RESET + "\n";
